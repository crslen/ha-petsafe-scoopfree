...
from petsafe_scoopfree import devices as Devices
...
class PetsafeEntity(CoordinatorEntity):
    """Generic Litter-Robot entity representing common data and methods."""

    def __init__(self, devices: Devices, entity_type: str, hub: PetSafeHub) -> None:
    ...
    def name(self) -> str:
        ...
        return f"{self.devices.name} {self.entity_type}"

    @property
    def unique_id(self) -> str:
        """Return a unique ID."""
        return f"{self.devices.serial}-{self.entity_type}"

    @property
    def device_info(self) -> DeviceInfo:
        """Return the device information for a Litter-Robot."""
        return DeviceInfo(
            identifiers={(DOMAIN, self.devices.serial)},
            manufacturer="Litter-Robot",
            model=self.devices.model,
            name=self.devices.name,
        )


class PetSafeControlEntity(PetsafeEntity):
    """A Litter-Robot entity that can control the unit."""

    def __init__(self, devices: Devices, entity_type: str, hub: PetSafeHub) -> None:
        """Init a Litter-Robot control entity."""
        super().__init__(devices=devices, entity_type=entity_type, hub=hub)
...
class PetsafeConfigEntity(PetSafeControlEntity):
